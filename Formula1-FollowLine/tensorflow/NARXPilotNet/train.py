...
from utils.narx_pilotnet import narx_pilotnet_model
...
if __name__=="__main__":
    ...
    dataset = process_dataset(path_to_data, type_image, data_type, img_shape)
    images_train = dataset[0]
    p_annotations_train = dataset[1]
    annotations_train = dataset[2]
    images_validation = dataset[3]
    p_annotations_validation = dataset[4]
    annotations_validation = dataset[5]
    
    timestr = time.strftime("%Y%m%d-%H%M%S")
    print(timestr)
    print(images_train.shape)
    print(annotations_train.shape)
    print(p_annotations_train.shape)
    print(images_validation.shape)
    print(annotations_validation.shape)
    print(p_annotations_validation.shape)
    ...
    model_name = 'narx_pilotnet'
    model = narx_pilotnet_model(img_shape, learning_rate = learning_rate)
    model_filename = timestr + '_narx_pilotnet_300_all_crop_no_seq_unique_albu_extreme_seq'
    model_file = model_filename + '.h5'

    AUGMENTATIONS_TRAIN, AUGMENTATIONS_TEST = get_augmentations(data_augs)

    # Training data
    train_gen = DatasetSequence(images_train, p_annotations_train, annotations_train, 
                                hparams['batch_size'], augmentations=AUGMENTATIONS_TRAIN)

    # Validation data
    valid_gen = DatasetSequence(images_validation, p_annotations_validation, annotations_validation, 
                                hparams['batch_size'], augmentations=AUGMENTATIONS_TEST)
